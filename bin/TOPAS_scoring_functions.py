...
def sum_weighted_z_scores(patient_dataframe, by):
    ...
    score_dataframe = score_dataframe.rename(
        columns={col: col.replace('weighted_', 'pat_') for col in score_dataframe.columns})
...
def topas_score_preprocess(results_folder, discard_isoforms=True):
    ...
    patients_proteins = patients_proteins.drop(patients_proteins.filter(regex='pat_|ref_').columns, axis=1)
    ...
    patients_zscores = patients_zscores.rename(
        columns={colname: colname.replace('zscore_', '') for colname in patients_zscores.columns})
