...
def compute_basket_scores_3rd_gen(
    ...
    for data_type in data_types:
        ...
        if debug:
            measures_ref = metrics.read_measures(
                results_folder, data_type, with_reference_channels=True
            )
            basket_scores_ref = calculate_basket_scores(
                measures_ref, basket_scores_ref, data_type
            )

    basket_scores = save_basket_scores(
        basket_scores, os.path.join(results_folder, "basket_scores.tsv")
    )
    if debug:
        save_basket_scores(
            basket_scores_ref, os.path.join(results_folder, "basket_scores_ref.tsv")
        )

    save_basket_scores_long_format(
        basket_scores, os.path.join(results_folder, "basket_scores_long.tsv")
    )
