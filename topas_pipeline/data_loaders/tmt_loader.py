...
def _scale_ms1_to_reference(df: pd.DataFrame, ref_channel_df: pd.DataFrame) -> pd.DataFrame:
    ...
    imputation_df = df.groupby(['Modified sequence', 'Charge'])['MS1 corrected reference intensity'].apply(lambda x: x.mean()).reset_index(
        name='Mean MS1 corrected reference intensity')

    df = pd.merge(left=df, right=imputation_df, on=['Modified sequence', 'Charge'], how='left')
...
def _impute_ms1_intensity(df: pd.DataFrame, ref_channel_df: pd.DataFrame) -> pd.DataFrame:
