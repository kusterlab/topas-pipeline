...
def filter_for_identification_metadata_columns(df: pd.DataFrame) -> pd.DataFrame:
    return df.filter(regex=rf"^({METADATA_COLUMN_PREFIX})")
...
def mark_quant_out_of_range(df, ratio_threshold: float = 0.01):
    ...
    logger.info(
        "Mark proteins with high dynamic range in the batch (lower than threshold)"
    )
    ...
    for batch in batches:
        ...
        df[metadata_cols.columns] += np.where(mask, "quan_OOR;", "")
...
def mark_peptide_id_out_of_range(df, threshold: int = 2):
    ...
    num_peptides = num_peptides.astype(str).applymap(
        lambda x: (
            re.search(pep_id_pattern, x).group(1)
            if re.search(pep_id_pattern, x)
            else None
        )
    )
    ...
    expanded = (
        pd.DataFrame(
            df["is_outlier_peptide_id"].tolist(),
            index=df.index,
            columns=metadata_cols_df.columns,
        )
        .fillna(False)
        .astype(bool)
    )
    ...
    df.loc[:, metadata_cols_df.columns] = df.loc[:, metadata_cols_df.columns].mask(
        expanded, df[metadata_cols_df.columns].astype(str) + "pepcount_OOR;"
    )
